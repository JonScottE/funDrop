...
def load_contract():
    ...
    # Load the contract ABI -------------- got ABI from contract deployment in remix
    with open(Path('artregistry_abi.json')) as f: # ---- Might want to rename artregistryabi file
...
def pin_artwork(artwork_name, artwork_file):
    ...
    ipfs_file_hash = pin_file_to_ipfs(artwork_file.getvalue()) ## problem here too  AttributeError: 'NoneType' object has no attribute 'getvalue'
...
address = st.selectbox("Select your account for funds to go to.", options=accounts)
st.markdown("---")

################################################################################
# Register New Artwork
################################################################################
st.markdown("##Start aFundraiser  Register New Cause")
artwork_name = st.text_input("Enter the name of the Cause")
artist_name = st.text_input("I don't think we need this Enter the artist name ")
initial_appraisal_value = st.text_input("Minimum Donation amount ")
file = st.file_uploader("Upload Artwork", type=["jpg", "jpeg", "png"])
if st.button("Register Artwork"):
    artwork_ipfs_hash = pin_artwork(artwork_name, file) ####somethings wrong here
